...
def test_min_list_positive(expression_project):
    logger = setup_test_logger("expression_min_list_test")
    logger.info("=" * 80)
    logger.info("[MIN_LIST_TEST] Тест выражения min_list/min_list_positive")
    logger.info("=" * 80)
    try:
        ...
        assert resp_json.get("result", {}).get("data", {}).get("minList") == 1
        ...
        payload_success = {
            "checks": {
                "object_result_checks": [
                    {"field": None, "path": ["data", "minList"], "type": "equal", "value": 1}
                ]
            },
        ...
        payload_fail = {
            "checks": {
                "object_result_checks": [
                    {"field": None, "path": ["data", "minList"], "type": "equal", "value": 2}
                ]
            },
            "input_data": {"a": 1, "b": 2, "c": [1, 2, 3]},
            "input_meta": {},
        ...
        logger.info("[COMPLETE] Тест min_list/min_list_positive завершён успешно")
    finally:
        logger.close()


@pytest.mark.usefixtures("expression_project")
def test_min_list_error(expression_project):
    logger = setup_test_logger("expression_min_list_test")
    logger.info("=" * 80)
    logger.info("[MIN_LIST_TEST] Тест выражения min_list/min_list_error (негативный)")
    logger.info("=" * 80)
    try:
        project_code, _ = expression_project
        file_panel_api = FilePanelAPI(project_code)
        ...
        logger.info("[COMPLETE] Тест min_list/min_list_error завершён успешно")
    finally:
        logger.close()
